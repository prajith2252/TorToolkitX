...
class MegaStatus(BaseStatus):
    ...
    async def update_now(self, get_msg=False):
        ...
        data = "torcancel megadl {} {}".format(
            self._downloader.get_gid(), self._sender_id
        )
                ...
                await self._update_message.edit(
                    ...
                    buttons=[
                        KeyboardButtonCallback(
                            "Cancel Mega Leech", data=data.encode("UTF-8")
                        )
                    ],
                )
    ...
    async def create_message(self):
        ...
        msg = "<b>Downloading:</b> <code>{}</code>\n".format(self._dl_task["name"])
        msg += "<b>Speed:</b> {}\n".format(human_readable_bytes(self._dl_task["speed"]))
        msg += "<b>Progress:</b> {} - {}%\n".format(
            self.progress_bar(
                (self._dl_task["completed_length"] / self._dl_task["total_length"])
            ),
            round(
                (self._dl_task["completed_length"] / self._dl_task["total_length"])
                * 100,
