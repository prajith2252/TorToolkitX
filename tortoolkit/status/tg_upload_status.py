...
class TGUploadStatus(BaseStatus):
    ...
    async def update_now(self, get_msg=False):
        ...
        if up_msg is not None:
            data = "upcancel {} {} {}".format(
                up_msg.chat_id, up_msg.id, self._sender_id
            )
    ...
    async def create_message(self):
        ...
        msg += " - {}%\n{} of {}\nSpeed: {}/s\nETA: {}\nUsing engine: Telethon".format(
            round(percent * 100, 2),
            human_readable_bytes(self._up_task.current_done),
            human_readable_bytes(self._up_task.current_total),
            human_readable_bytes(self._up_task.current_speed),
            self._up_task.current_eta,
        )
