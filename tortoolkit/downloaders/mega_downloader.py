...
from megasdkrestclient import MegaSdkRestClient, constants, errors
...
class MegaDownloader(BaseTask):
    ...
    async def execute(self):
        ...
        try:
            dl_add_info = await self._aloop.run_in_executor(
                None, partial(mega_client.addDl, self._link, path)
            )
        ...
        dl_info = await self._aloop.run_in_executor(
            None, partial(mega_client.getDownloadInfo, dl_add_info["gid"])
        )
        ...
        while True:
            ...
            if (dl_info["total_length"] / (1024 * 1024 * 1024)) > get_val(
                "MAX_MEGA_LIMIT"
            ):
        ...
        if self._mega_down.is_errored or self._mega_down.is_canceled:
            await self._update_msg.edit(
                "Your Task was unsccuessful. {}".format(
                    self._mega_down.get_error_reason()
                ),
                buttons=None,
            )
            return False
            ...
            if self._mega_down.is_completed:
                await self._update_msg.edit(
                    self._mega_down.get_error_reason(), buttons=None
                )
