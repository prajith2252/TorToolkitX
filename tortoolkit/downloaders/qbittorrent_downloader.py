...
class QbittorrentDownloader(BaseTask):
    ...
    async def get_client(
        self, host=None, port=None, uname=None, passw=None, retry=None
        ...
        try:
            ...
            await self._aloop.run_in_executor(
                None, client.application.set_preferences, qbaconf
            )
            torlog.debug(
    ...
    async def get_torrent_info(self, client=None, ext_hash=None):
        ...
        if ext_hash is None:
            ...
            return await self._aloop.run_in_executor(
                None, partial(client.torrents_info, torrent_hashes=ext_hash)
            )
    ...
    async def add_torrent_magnet(self):
        ...
        client = await self.get_client()
        try:
            len(await self.get_torrent_info(client))
            ...
            op = await self._aloop.run_in_executor(
            ...
            if op.lower() == "ok.":
                ...
                while True:
                    if (datetime.now() - st).seconds >= 10:
                        ...
                        self._is_errored = True
                        self._error_reason = (
                            "The torrent was not added due to an error."
                        )
    ...
    async def add_torrent_file(self):
        ...
        try:
            ...
            op = await self._aloop.run_in_executor(
                None,
                partial(client.torrents_add, torrent_files=[path], save_path=savepath),
            )
    ...
    async def pause_all(self):
        ...
        msg = ""
        tors = await self._aloop.run_in_executor(
    ...
    async def deregister_torrent(self, ext_hash=None):
        ...
        await self._aloop.run_in_executor(
            None,
            partial(client.torrents_delete, torrent_hashes=ext_hash, delete_files=True),
        )
    ...
    async def register_torrent(self):
        ...
        if not self._is_file:
            ...
            if torrent.progress == 1 and torrent.completion_on > 1:
                self._is_errored = True
                self._error_reason = (
                    "The provided torrent was already completely downloaded."
                )
                return False
    ...
    async def update_progress(self, except_retry=0, sleepsec=None):
        ...
        while True:
            ...
            if tor_info.size > (get_val("MAX_TORRENT_SIZE") * 1024 * 1024 * 1024):
                self._is_errored = True
                self._error_reason = "Torrent oversized max size is {}. Try adding again and choose less files to download.".format(
                ...
                try:
                    ...
                    if tor_info.state == "uploading" or tor_info.state.lower().endswith(
                        ...
                        await self._aloop.run_in_executor(
                            None,
                            partial(
                                (await self.get_client()).torrents_pause, tor_info.hash
                            ),
                        )
...
class QbitController:
    ...
    async def execute(self):
        ...
        if self._is_file:
            self._update_message = await self._user_msg.reply(
                "Downloading the torrent file."
            )

            torrent = await self._entity_msg.download_media()
            self._qbit_task = QbittorrentDownloader(
                torrent, self._user_msg.sender_id, self._is_file
            )
            ...
            if self._qbit_task.is_errored:
                self._update_message(
                    "Your Task was unsccuessful. {}".format(
                        self._qbit_task.get_error_reason()
                    )
                )
                return False
            ...
            self._qbit_task = QbittorrentDownloader(
                torrent, self._user_msg.sender_id, self._is_link
            )
        ...
        self._update_message = await self._update_message.edit(
            ...
            buttons=[
                ...
                [
                    KeyboardButtonCallback(
                        ...
                        data=f"doneselection {self._user_msg.sender_id} {self._user_msg.id}".encode(
                            "UTF-8"
                        ),
                    )
                ],
        ...
        self._update_message = await self._update_message.edit(
            buttons=[KeyboardButtonCallback("Cancel Leech", data=data.encode("UTF-8"))]
        )
        ...
        if self._qbit_task.is_errored or self._qbit_task.is_canceled:
            ...
            if self._qbit_task.is_completed:
                await self._update_message.edit(
                    "{}".format(self._qbit_task.get_error_reason()), buttons=None
                )
            return result
