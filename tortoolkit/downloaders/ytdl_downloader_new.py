...
class YTDLDownloader(BaseTask):
    ...
    async def execute(self):
        ...
        if os.path.exists(path):
            with open(path, encoding="UTF-8") as file:
                ...
                await get_max_thumb(ytdata, self._suid)
    ...
    def progress_update(self, progress):
        if self._cancel_msg is not None:
            if self._updb.get_cancel_status(
                self._cancel_msg.chat_id, self._cancel_msg.id
            ):
                print("canceled")
...
class YTDLController:
    ...
    async def execute(self):
        ...
        await (await self.get_update_message()).edit(
            "Youtube Download is complete. Success/Failure is not determinded.",
            buttons=None,
        )
        ...
        if ytdl_task.is_errored:
            if res is False:
                ...
                omess.edit(
                    ...
                    + str(ytdl_task.get_error_reason()),
                    buttons=None,
                )
                await self.user_message.reply(
                    "Something went wrong, try again later."
                    + str(ytdl_task.get_error_reason())
                )
...
class PYTDLController:
    ...
    async def execute(self):
        ...
        await (await self.get_update_message()).edit(
            "Youtube Playlist Download is complete. Success/Failure is not determinded.",
            buttons=None,
        )
