...
class RcloneUploader(BaseTask):
        ...
        while True:
            ...
            if os.path.isdir(path):
                ...
                rclone_pr = subprocess.Popen(
                    rclone_copy_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )
                ...
                if get_val("ENABLE_SA_SUPPORT_FOR_GDRIVE") and is_gdrive:
                    while True:
                        ...
                        if data == "":
                            ...
                            mat = re.findall(
                                ".*User.*Rate.*(Limit|Quota).*Exceeded.*",
                                data,
                                re.IGNORECASE,
                            )
                            if mat is not None:
                                if len(mat) > 0:
                                    is_rate_limit = True
                                    torlog.info(
                                        "Current account limit reached now changing the SA account."
                                    )
                ...
                if is_gdrive:
                    gid = await self.get_glink(
                        dest_drive, dest_base, os.path.basename(path), conf_path
                    )
                    ...
                    if ".sharepoint.com/" in get_val("ONEDRIVE_BASE_FOLDER_URL"):
                        folder_link = get_val("ONEDRIVE_BASE_FOLDER_URL") + parse.quote(
                        ...
                        ids = gid[1].split("#")
                        folder_link = "https://onedrive.live.com/?cid={}&id={}".format(
                            ids[0], ids[1]
                        )
                    ...
                    if od_index:
                        index_link = "{}/{}/".format(
                            od_index.strip("/"), parse.quote(os.path.basename(path))
                        )
                        index_link = requote_uri(index_link)
                        torlog.info("index link " + str(index_link))
                    self._error_reason = "Uploaded Size:- {}\nUPLOADED FOLDER :-<code>{}</code>\nTo OneDrive.".format(
                    ...
                    self._error_reason = "Uploaded Size:- {}\nUPLOADED FOLDER :-<code>{}</code>\nTo {}.".format(
                        ul_size, os.path.basename(path), general_drive_name
                    )
                ...
                if is_gdrive:
                    ...
                    if ".sharepoint.com/" in base_url:
                        ...
                        file_link = "{}&parent={}".format(
                            file_link, base_url.split("?id=")[-1]
                        )
                        ...
                        ids = gid[1].split("#")
                        file_link = "https://onedrive.live.com/?cid={}&id={}".format(
                            ids[0], ids[1]
                        )
                ...
                TtkUpload().deregister_upload(
                    self._user_msg.chat_id, self._user_msg.id
                )  # deregister the upload here
                if not is_rate_limit:
    ...
    async def gen_sa_rc_file(self, change=False):
        ...
        if sa_td_id != "":
            ...
            raise Exception(
                "SA Enabled but not configured correctly as no sa teamdrive id or folder is provided."
            )
    ...
    async def rclone_process_update(self):
        ...
        while True:
            ...
            if mat is not None:
                if len(mat) > 0:
                    ...
                    if time.time() - start > edit_time:
                        ...
                        nstr = nstr.split(",")
                        prg = nstr[1].strip("% ")
                        self._current_update = self.RcloneStatus(
                            nstr[0], nstr[2], nstr[3], prg
                        )
            ...
            if sleeps:
                if TtkUpload().get_cancel_status(
                    self._user_msg.chat_id, self._user_msg.id
                ):
                    return False
    ...
    async def get_glink(self, drive_name, drive_base, ent_name, conf_path, isdir=True):
        ...
        filter_path = os.path.join(
            os.getcwd(), str(time.time()).replace(".", "") + ".txt"
        )
...
class RcloneController:
    ...
    async def execute(self):
        if self._previous_task_msg is not None:
            self._update_msg = await self._previous_task_msg.reply(
                "Starting the rclone upload."
            )
        else:
        ...
        if self._rclone_up.is_errored:
            await self._update_msg.edit(
                "Your Task was unsccuessful. {}".format(
                    self._rclone_up.get_error_reason()
                ),
                parse_mode="html",
            )
        else:
            ...
            buttons = [[KeyboardButtonUrl("Drive URL", drive_link)]]
            ...
            await self._user_msg.reply(
                self._rclone_up.get_error_reason(), buttons=buttons, parse_mode="html"
            )
