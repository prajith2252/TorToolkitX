...
from telethon.tl.types import DocumentAttributeFilename, KeyboardButtonCallback
...
from ..downloaders.ytdl_downloader import PYTDLController, YTDLController
from ..downloaders.ytdl_downloader_new import PYTDLController as PYTDLControllerNew
from ..downloaders.ytdl_downloader_new import YTDLController as YTDLControllerNew
from ..uploaders.archiver import Archiver
from ..uploaders.extractor import Extractor
from ..uploaders.rclone_uploader import RcloneController, RcloneUploader
...
class TaskSequence:
        ...
        if self._task_type == self.LEECH:
            ...
            if not choices["rclone"]:
                if not get_val("LEECH_ENABLED"):
                    await self._user_msg.reply(
                        "Leech to Telegram is disabled by admin."
                    )
                    return
            else:
                if not get_val("RCLONE_ENABLED"):
                    await self._user_msg.reply(
                        "Leech to Rclone Drive is disabled by admin."
                    )
                    return
            ...
            if not choices["rclone"]:
                ...
                rcloneup = RcloneController(
                    dl_path, self._user_msg, prev_update_message, self._dest_drive
                )
            ...
            if up_dest == "tg":
                teleup = TelegramUploader(
                    dl_path, self._user_msg, await self._entity_message.get_message()
                )
                files = await teleup.execute()
                # temp:
                print(files)
                ...
                rcloneup = RcloneController(
                    dl_path, self._user_msg, self._entity_message, self._dest_drive
                )
                await rcloneup.execute()
        ...
        if self._entity_message is None:
            ...
            for i in self._entity_message.document.attributes:
                ...
                if name is None:
                    ...
                    return QbitController(
                        self._user_msg, self._entity_message, is_file=True
                    )
            ...
            if raw_text.lower().startswith("magnet:"):
                ...
                return QbitController(
                    self._user_msg, self._entity_message, is_link=True
                )
                ...
                if get_val("MEGA_ENABLE"):
                    if "folder" in raw_text or "/#F!" in raw_text:
                        if get_val("ALLOW_MEGA_FOLDER"):
                            ...
                            await self._user_msg.reply(
                                "Mega folder leeching is disabled by admin."
                            )
                            return
                    ...
                    if get_val("ALLOW_MEGA_FILES"):
                        ...
                        await self._user_msg.reply(
                            "Mega file leeching is disabled by admin."
                        )
                        return
    ...
    async def get_leech_choices(self):
        ...
        if rcl_config is not None and get_val("RCLONE_ENABLED"):
            if get_val("SHOW_REMOTE_LIST"):
                ...
                for j in conf.sections():
                    ...
                    if "team_drive" in list(conf[j]):
                        buts.append(
                            [
                                KeyboardButtonCallback(
                                    f"{j} - TD", f"leechselect drive {tsp} {j}"
                                )
                            ]
                        )
                    else:
                        buts.append(
                            [
                                KeyboardButtonCallback(
                                    f"{j} - ND", f"leechselect drive {tsp} {j}"
                                )
                            ]
                        )
            else:
                buts.append(
                    [
                        KeyboardButtonCallback(
                            "To Drive", data=f"leechselect drive {tsp}"
                        )
                    ]
                )
        ...
        conf_mes = await self._user_msg.reply(
        ...
        res = {"rclone": rclone, "zip": is_zip, "ext": is_ext}
    ...
    async def get_leech_choice(self, e, timestamp):
        ...
        lis = [False, None]
        cbak = partial(
            self.get_leech_choice_callback, o_sender=e.sender_id, lis=lis, ts=timestamp
        )
    ...
    async def get_zip_choice(self, e, timestamp, lis=None, start=True, ext=False):
        ...
        if start:
            cbak = partial(
                self.get_leech_choice_callback,
                o_sender=e.sender_id,
                lis=lis,
                ts=timestamp,
            )
            lis[2] = cbak
            if ext:
                e.client.add_event_handler(
                    cbak, events.CallbackQuery(pattern="leechzipex")
                )
            else:
                e.client.add_event_handler(
                    cbak, events.CallbackQuery(pattern="leechzip")
                )
            return lis
        else:
