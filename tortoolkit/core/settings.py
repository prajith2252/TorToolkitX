...
async def handle_setting_callback(e):
    ...
    if cmd[1] == "fdocs":
        ...
        await handle_settings(
            mmes,
            True,
            "\nWelcome to Mega.nz Config Menu.",
            submenu="meganzmenu",
            session_id=session_id,
        )
    elif cmd[1] == "mainmenu":
        ...
        await handle_settings(
            mmes,
            True,
            f"<b><u>Changed the value to {val} of Mega Enabled.</b></u>",
            "meganzmenu",
            session_id=session_id,
        )
        ...
        await handle_settings(
            mmes,
            True,
            f"<b><u>Changed the value to {val} of Mega Folder Enabled.</b></u>",
            "meganzmenu",
            session_id=session_id,
        )
        ...
        await handle_settings(
            mmes,
            True,
            f"<b><u>Changed the value to {val} of Mega Files Enabled.</b></u>",
            "meganzmenu",
            session_id=session_id,
        )
        ...
        await handle_settings(
            mmes,
            True,
            f"<b><u>Changed the value to {val} of Show rmeote list.</b></u>",
            "rclonemenu",
            session_id=session_id,
        )
        ...
        # answer message, type handler, value
        await e.answer(
            "Type the new value for MAX Mega SIZE. Note that integer is expected.",
            alert=True,
        )
        ...
        await general_input_manager(
            e, mmes, "MAX_MEGA_LIMIT", "int", val, db, "meganzmenu"
        )
...
async def handle_settings(e, edit=False, msg="", submenu=None, session_id=None):
    ...
    if submenu is None:
        ...
        if get_val("ENABLE_SA_SUPPORT_FOR_GDRIVE"):
            ...
            menu.append(
                [
                    KeyboardButtonCallback(
                        f"{prev} LOADED SAS",
                        f"settings change_drive sas_acc {session_id}",
                    )
                ]
            )
        ...
        await get_bool_variable(
            "MEGA_ENABLE", "Enable mega.nz leech.", menu, "meganzenable", session_id
        )
        await get_bool_variable(
            "ALLOW_MEGA_FOLDER", "Enable folder leech.", menu, "megafolder", session_id
        )
        await get_bool_variable(
            "ALLOW_MEGA_FILES", "Enable files leech.", menu, "megsfiles", session_id
        )
